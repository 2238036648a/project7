package com.mr.bizdiscount.redis;
...
public class RedisUtils {
    ...
    public CommonResult setMapValue(String key, Map map) {
        try {
            Object data = redisTemplate.opsForHash().get(key,map);
            return CommonResultUtils.getResult(ErrorEnum.SUCCESS_RESTUL.getErrCode(), ErrorEnum.SUCCESS_RESTUL.getErrMassage(), data);
        } catch (Exception e) {
    ...
    public void delHAashEntry(String key) {

        try {
            expire(key,1,TimeUnit.MILLISECONDS);
        } catch (Exception e) {
            e.printStackTrace();
            log.error("redis expire set error",e);
        }
    }
    /**
     * 判断key是否存在
